    ...
    .then(function(post) {
      ...
      `
        ...
        function updateMessage() {
            ...
            if (screenWidth < 576) {
                $(".header__action-search").addEventListener('click', function() {
                    $(".search_mobile").classList.toggle("none");
                    this.classList.toggle("select-input-mobile")
                ...
                $(".menu-mobile_title-icon").addEventListener('click', function() {
                    $(".menu-mobile").classList.remove("show-menu");

                })

            } else if (screenWidth < 1000) {} else {

            }
        }

        // Gọi hàm cập nhật thông điệp khi kích thước màn hình thay đổi
        updateMessage();
    })
    .catch(function(err) {
        console.log(err);
